...
- Regression smoke pages should show a final summary with passed/total count, expected build tag, loaded build tag, and schema version; short and long Git hashes may match by prefix.
...
- The current character archive was externally audited on 2026-06-10. Every current `HISTORICAL_PEOPLE` entry must have one matching `HISTORICAL_AUDIT_ROWS` entry, and static tests must fail if a new person lacks audit metadata.
- For new characters, add the natal-data source, rating, URL, time-confidence derivation, zone reliability, and audit date through the structured audit layer; update `docs/historical-character-audit.md` at the same time.
...
- Public archive policy is currently strict: production historical examples should be audited. `partial` and `pending` states exist for staging, review workflows, or explicitly approved future exceptions, not as the default public set.
