...
const interpretationBlock = sectionBetween(app, "function interpretChart(chart)", "function renderScoreBreakdown(focuses)");
...
assert("Visible reading blocks expose practical conclusions", conclusionFunctions.every((name) => app.includes(`function ${name}`)) && conclusionFunctions.every((name) => interpretationBlock.includes(`conclusion: ${name}`)));
