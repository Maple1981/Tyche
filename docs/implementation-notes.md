...
Chart calculation triggers should show a visible loading/progress state before entering synchronous calculation work. The progress indicator should sit in a fixed viewport layer so it is visible from both the form and the historical-person modal flow. The form submit path and historical-example path should both use the shared calculation task flow: set busy state, show the progress indicator, mark the triggering button busy/disabled, await one animation frame so the UI can paint, then run calculation and clean up the loading state immediately after the real work finishes. Do not add artificial minimum display delays; fast calculations may complete before the loading state is perceptible.
