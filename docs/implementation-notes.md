...
Within each binding group, prefer named handlers for non-trivial behavior. Tab activation, preference toggles, modal clicks, popover document interactions, birthplace keyboard navigation, clearing place fields, date/time changes, and chart-form submission should live in named functions so event binding remains easy to scan. Event groups should bind through shared node/element/document/window listener adapters instead of calling `addEventListener` directly throughout the shell. Avoid inline event lambdas when a behavior is reused or has branching logic.
